...
/// Capture ingress and deployment observations around the provider-free
/// durable provenance record.
pub(crate) fn capture_service_request_provenance(
    command: &Value,
    request_id: &str,
    job_id: &str,
    connection_instance_id: &str,
    runtime_lane_id: &str,
) -> ServiceRequestProvenance {
    ...
    let identity_assurance =
        normalize_identity_assurance(optional_string(command, "identityAssurance").as_deref());

    ServiceRequestProvenance {
        schema_version: SERVICE_REQUEST_PROVENANCE_SCHEMA_VERSION.to_string(),
        request_id: optional_string(command, "requestId").unwrap_or_else(|| request_id.to_string()),
