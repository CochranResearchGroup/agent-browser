...
use super::service_model::{
    BrowserHealth, RemoteViewHandoff, ServiceState, TabLifecycle, ViewStreamProvider,
};
...
/// Prefer a retained route for an already-live browser even when rollback has
...
pub fn apply_available_retained_remote_view_route(
    ...
    let route_owner_matches = match (route.browser_id.as_deref(), route.session_id.as_deref()) {
        (Some(owner_browser_id), Some(owner_session_id)) => {
            owner_browser_id == browser_id && owner_session_id == session_name
        }
        (None, None) => true,
        _ => false,
    };
...
mod tests {
    ...
    use crate::native::service_model::{
        BrowserProcess, DisplayAllocation, RemoteViewAcquisitionLease, RemoteViewRoute,
        RoutePoolEntry, ServiceState, ViewStreamProvider,
    };
    ...
    #[test]
    fn handoff_resolution_prefers_an_available_retained_route_for_a_live_owner() {
        let handoff = RemoteViewHandoff {
            id: "job-handoff-a".to_string(),
            intent: json!({ "viewStreamProvider": "rdp_gateway" }),
            browser_id: Some("session:browser-a".to_string()),
            session_name: Some("session-a".to_string()),
            view_stream_provider: Some(ViewStreamProvider::RdpGateway),
            last_route_id: Some("route-b".to_string()),
            last_route_pool_entry_id: Some("pool-b".to_string()),
            last_display_allocation_id: Some("display-b".to_string()),
            ..RemoteViewHandoff::default()
        };
        let state = ServiceState {
            display_allocations: BTreeMap::from([(
    ...
    #[test]
    fn acquisition_preserves_active_quarantine_before_reserving_new_route() {
        ...
        let error = begin_route_bound_handoff_plan_acquisition(
            &repository,
            None,
            &acquisition_plan,
            "session:research",
        ...
        assert!(error.contains("route_bound_acquisition_quarantined"));
        let state = store.load().unwrap();
        assert_eq!(
            state.remote_view_acquisition_leases["lease-a"].phase,
            "rollback_incomplete"
        );
        assert_eq!(state.remote_view_routes["route-a"].state, "orphaned");
        assert_eq!(state.display_allocations["display-a"].state, "orphaned");
        assert_eq!(state.route_pool["pool-b"].state, "available");
