...
async fn write_json_result(
    ...
    let (status, resp_body) = json_result_parts(result, error_status);
    let response = format!(
        "HTTP/1.1 {status}\r\nContent-Type: application/json; charset=utf-8\r\nContent-Length: {}\r\nConnection: close\r\n{CORS_HEADERS}\r\n",
        resp_body.len()
    );
...
fn json_result_parts(result: Result<String, String>, error_status: &str) -> (&str, String) {
    match result {
        Ok(resp) => ("200 OK", resp),
        Err(e) => (
            error_status,
...
pub(super) fn service_request_relay_session(
    ...
    if !matches!(
        ...
        for value in SERVICE_REQUEST_HTTP_RELAY_CANONICAL_POINTERS
            .iter()
            .map(|(_, pointer)| request.pointer(pointer))
            .chain([
...
pub(crate) const SERVICE_REQUEST_HTTP_RELAY_CANONICAL_POINTERS: &[(&str, &str)] = &[
    ("sessionName", "/sessionName"),
    ("browserId", "/browserId"),
    ("serviceTabHandle", "/serviceTabHandle/sessionName"),
    ("serviceTabHandle", "/serviceTabHandle/browserId"),
];
...
mod tests {
    ...
    #[test]
    fn service_request_command_parses_json_and_adds_http_request_id() {
    ...
    #[test]
    fn service_request_command_preserves_http_legacy_args_precedence() {
        let command = service_request_command(
            r##"{"action":"navigate","params":{"args":["--from-params"]},"args":{"raw":true}}"##,
        )
        .unwrap();

        assert_eq!(command["args"], json!({"raw": true}));
    }
    ...
    #[test]
    fn desktop_capture_browser_identity_does_not_select_a_daemon_lane() {
        let body = r##"{"action":"desktop_capture","browserId":"browser-rdp-1","serviceName":"desktop-observer","agentName":"codex","taskName":"capture"}"##;
        let command = service_request_command(body).unwrap();
        ...
        let narrowed = r##"{"action":"desktop_capture","browserId":"browser-rdp-1","sessionName":"rdp-session","serviceName":"desktop-observer","agentName":"codex","taskName":"capture"}"##;
        let command = service_request_command(narrowed).unwrap();
        assert_eq!(
            service_request_relay_session("AgentBrowserDashboard", narrowed, &command),
            "rdp-session"
        );
    }

    #[test]
    fn desktop_locate_browser_identity_does_not_select_a_daemon_lane() {
        let body = r##"{"action":"desktop_locate","browserId":"browser-rdp-1","locator":{"locatorId":"p110-control-v1"},"serviceName":"desktop-observer","agentName":"codex","taskName":"locate"}"##;
        let command = service_request_command(body).unwrap();
        ...
        let narrowed = r##"{"action":"desktop_locate","browserId":"browser-rdp-1","sessionName":"rdp-session","locator":{"locatorId":"p110-control-v1"},"serviceName":"desktop-observer","agentName":"codex","taskName":"locate"}"##;
        let command = service_request_command(narrowed).unwrap();
        assert_eq!(
            service_request_relay_session("AgentBrowserDashboard", narrowed, &command),
            "rdp-session"
