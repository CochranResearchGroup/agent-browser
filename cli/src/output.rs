...
Notes:
  ...
  - tab_handle_refresh reuses only a compatible target with canonical Service State custody for the same caller. open_if_missing persists a canonical replacement when needed. replace_duplicates closes only same-caller targets and preserves foreign or unattributed peers. Read duplicateCleanupAttempted, peerCleanupAttempted, and duplicateTargetCleanup from every response.
