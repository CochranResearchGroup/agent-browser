...
Notes:
  ...
  - service leases returns principal-scoped profile leases with exact owner generation, subordinate work, blocking identity axes, authorized actions, typed recourse, and doctor findings. An authenticated rejoin can repair one unproven live session only when it is the unique exact session of the capability-bound owner. If a replay advanced that owner generation, rejoin compare-and-swaps only the same principal, profile, and capability binding to the current ready generation, then binds only that session and its same-browser active tabs through the supplied expiry. After an unexpected browser exit, a non-expired registered session work lease preserves exact profile selection while replay establishes the next owner; labels and legacy observations do not.
...
Notes:
  ...
  - Principal-bound profile reuse accepts an optional profile capability only through HTTP Authorization: Bearer headers or the ephemeral MCP profileCapability field on service_access_plan and service_request. Caller labels do not grant authority. Matching principals reuse their exact lane; foreign principals wait; unauthenticated callers authenticate; and contradictory same-principal profile evidence returns lifecycle_profile_identity_inconsistent. Never place capabilities in URLs, request bodies, command-line arguments, logs, or persisted Service State.
