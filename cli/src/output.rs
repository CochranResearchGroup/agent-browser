...
Notes:
  ...
  - For interactive authentication, service request action service_profile_manual_seeding_acquire reserves the exact registered profile and a ready keeper slot in SQLite, opens one headed browser without CDP, and returns an opaque handoff after presentation proof. Handoff resolution rechecks process and route visibility. An uncertain launch remains reserved while its PID exists; once observed absent, acquire returns retryRequiresNewOperation=true without launching. Acquiring or resolving a previously ready browser that exited closes its handoff and returns the same marker. A new request ID is required for another launch. service_profile_manual_seeding_close requires the exact returned profileId, targetServiceId, handoffId, and pid; it stops that verified process, closes the handoff, retains the keeper route, and returns attachable relaunch and separate authentication-probe guidance.
