...
Notes:
  ...
  - diagnostics responses include controlPlaneAttestation. Treat complete=true as the only effect-authority success; missingProofs identifies absent or conflicting owner-generation, process-identity, current exclusive-profile-lease, or owner-custody proof. ownerCustody requires a current transfer receipt or exact managed-launch lifecycle evidence for this boot, generation and process; old active handle lease metadata cannot override current state. Retained-browser profile aliases require the configured catalog/runtime name, exact current owner, physical directory and browser endpoint to agree.
