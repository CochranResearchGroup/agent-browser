...
Notes:
  ...
  - Service-scoped launches reject active exclusive profile conflicts by default before browser start; set profileLeasePolicy=wait and profileLeaseWaitTimeoutMs to wait for release instead. Same-session retained browser reuse remains allowed.
...
Notes:
  ...
  - Session records expose profileSelectionReason as authenticated_target, target_match, service_allow_list, explicit_profile, or null, and expose profileLeaseDisposition as new_browser, reused_browser, active_lease_conflict, or null. browser_launch_recorded events mirror those values in details when known, and service-scoped launches reject or wait on active exclusive profile conflicts according to profileLeasePolicy.
