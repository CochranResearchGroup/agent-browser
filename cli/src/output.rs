...
Notes:
  ...
  - Complete browserId/sessionName reuse hints are validated against the current access plan. A matching pair attaches the exact compatible retained browser; a contradictory pair fails before browser effects. Cold shared-local plans use a stable shared-profile-* session instead of inheriting the runtime host's ambient lane.
