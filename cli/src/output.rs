...
Notes:
  ...
  - service_request accepts one intent object with serviceName, agentName, taskName, siteId/loginId/accountId/url, targetServiceId, profile or runtimeProfile hints, profileLeasePolicy, profileLeaseWaitTimeoutMs, action, params, and jobTimeoutMs, then queues the browser command through the same service-owned control path. Use action=cdp_free_launch when the request should launch and track a headed browser without a DevTools port.
  ...
  - HTTP GET /api/service/profiles/lookup and MCP agent-browser://profiles/lookup{?serviceName,targetServiceId,targetServiceIds,siteId,siteIds,loginId,loginIds,accountId,accountIds,url,readinessProfileId} apply the authoritative service profile selector for serviceName plus targetServiceId, siteId, loginId, accountId, url, or their array aliases and return the selected profile, reason, readiness, readiness summary, and seedingHandoff when manual seeding is required.
