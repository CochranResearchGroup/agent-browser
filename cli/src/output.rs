...
Notes:
  ...
  - Managed Guacamole routes with providerMode=simultaneous_view use a transient connection-sharing credential so another dashboard joins the existing tunnel instead of opening another RDP login. The credential is iframe-local and must never be exposed as an operator handoff or persisted as evidence.
