...
Notes:
  ...
  - Configured remote-view reattach and route-switch operations reserve the exact bound presentation slot at recovery priority before route mutation and release it afterward. Route switch uses an atomic migration reservation: the moving browser may retain its active source slot, an occupied destination stays bound until parking, and checkout transfers the leased destination before release. Their recoveryAdmission and recoveryRelease fields are the capacity receipts; not_configured preserves older-state compatibility but is not admission proof.
