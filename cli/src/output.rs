...
Notes:
  ...
  - service recovery acquire is the capability-bound high-level entry point. On Linux it enrolls the exact profile with the protected root authority, acquires one broker-managed ephemeral claim, consumes one in-process browser-launch permit, and commits the launched PID plus owner generation before returning acquired. The response exposes only leaseAuthority claim and owner receipts; it never exposes or serializes an executable bearer. Retained Service State sessions, owners, transitions, and warnings cannot select the route or block this acquisition. Completion failure closes the launched browser and terminalizes the consumed intent as uncertain before any retry. A confirmed close reconciles the exact protected owner; failed or unproven shutdown retains it. The daemon route is derived from the protected principal and profile and cannot be selected by the client. Non-Linux builds retain the compatibility acquisition path until the protected authority service is ported.
  ...
  - Text service browsers focuses retained browser records and their lastHealthObservation fields. Its JSON response also returns protectedBrowserOwnerObservations keyed by browser ID when a protected launch receipt was projected. These records are freshness-bounded observations with operationalAuthority=false. Revalidate them through the protected root authority before adoption, cleanup, or any other effect.
