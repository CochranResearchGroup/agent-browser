...
Notes:
  ...
  - The bounded events log records reconciliation summaries, browser launch metadata including profileSelectionReason and profileLeaseDisposition when known, browser health transitions, browser recovery starts, and tab lifecycle changes.
  ...
  - POST /api/service/request accepts one intent object with serviceName, agentName, taskName, siteId/loginId, targetServiceId, profile or runtimeProfile hints, action, params, and jobTimeoutMs, then queues the browser command through the same service-owned control path.
  ...
  - Service-scoped launches fail before browser start when the selected profile is already held by another exclusive session; same-session retained browser reuse remains allowed.
...
Notes:
  ...
  - Session records expose profileSelectionReason as authenticated_target, target_match, service_allow_list, explicit_profile, or null, and expose profileLeaseDisposition as new_browser, reused_browser, active_lease_conflict, or null. browser_launch_recorded events mirror those values in details when known, and service-scoped launches reject active exclusive profile conflicts before starting another browser.
