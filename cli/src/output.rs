...
Usage:
  ...
  agent-browser service leases <lease-id> recover plan --revision <lease-revision> --capability-file /absolute/private/path/fedex.cap
  agent-browser service leases <lease-id> recover apply --capability-file /absolute/private/path/fedex.cap --plan-file /absolute/path/plan.json
...
Notes:
  ...
  - Strict canonical claims advertise recover_plan. Recovery planning has no effect and returns a two-minute authorization bound to the exact claim revision, fence, controller, operation, and requested recovery tenure. Recovery apply revalidates that authorization in the serialized authority mutation, advances the claim revision and resource fence, and returns an idempotent receipt. Exact completed replay returns the original receipt after controller rotation or revocation and never recreates expired authority.
  - service recovery acquire is the capability-bound high-level entry point. It returns acquired, recovery_available, or blocked without accepting a client-selected daemon route. An acquired result includes the canonical leaseClaim, exact leaseEffectAuthorization, durable leaseAcquisitionReceipt, and leaseAcquisitionReplayed flag. The service selects a five-minute ephemeral claim expiry. Exact replay grants no new authority after expiry; another operation from the same capability may join the current claim without renewing it or minting another fence. The daemon validates the envelope immediately before launch. Effect authorization v5 is signed with the private Ed25519 lease-authority root and binds the current capability revision, browser_launch action class, exact daemon-session audience, operation ID, claim revision, fence, signing-key epoch, and two-minute window. Executors load only the versioned public verification keyring and cannot mint authority or accept an unknown or future signing epoch. Signer and verifier files become current as one digest-verified selected trust generation; standalone legacy key files require explicit migration and cannot bootstrap a parallel trust root. Copy operationIdempotencyKey to leaseEffectOperationId on the planned launch. Capability revocation, scope alteration, key substitution, epoch mismatch, or expiry prevents admission. Do not log or project the bearer. If the public verifier keyring survives private-key loss, issuance returns lease_authority_signing_key_recovery_required and does not create a replacement signer. It reuses an exact current principal lane, automatically applies only conclusive terminal-owner recovery, retries acquisition once, and hard-blocks a current foreign principal. Unproven or inconsistent principal/profile identity returns a reviewed reconciliation plan without launching a duplicate browser. The lower-level plan, apply, and status commands remain available for explicit operator review.
...
Environment:
  ...
  AGENT_BROWSER_PROFILE_LEASE_MODE
                                 Emergency legacy service acquisition mode. Set to fail_open_ephemeral to redirect only duplicate or exclusively leased requests without canonical lease authorization to isolated managed-one-time profiles. Canonical authorizations are never rewritten. Authentication is not preserved.
