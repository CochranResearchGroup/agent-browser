...
Subcommands:
  ...
  remote-view           Diagnose install, Guacamole, XRDP, RDP user, privilege, config, route-display, and display-access state
...
Configuration:
  ...
  Set service.defaultBrowserBuild to stealthcdp_chromium after executablePath, AGENT_BROWSER_EXECUTABLE_PATH, or service.browserBuildManifests.stealthcdp_chromium.manifestPath points at the patched Chromium artifact. Ordinary launches consume that default through the service access-plan resolver unless the caller explicitly supplies a profile, browser host, headless mode, executable, or browser build. If no explicit default is configured and a ready stealthcdp_chromium manifest is available, fresh installs prefer that build automatically.
