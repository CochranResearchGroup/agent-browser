...
Notes:
  ...
  - Browser-mode file_transfer preserves existing context download settings, subscribes through a temporary empty context disposed before clicking, and matches the authorized frame and GUID. Completed files are copied through verified browser process identity (including its Linux mount namespace); sources and existing destinations are preserved. Newly owned Chrome uses the configured download path or profile-local agent-browser-downloads directory. Typed subscription and artifact errors retain trace guidance; inspect effects before retry.
