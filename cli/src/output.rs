...
Notes:
  ...
  - POST /api/service/site-policies/<id> and POST /api/service/providers/<id> persist service config records. DELETE on the same paths removes persisted records.
...
Notes:
  ...
  - service_site_policy_upsert, service_site_policy_delete, service_provider_upsert, and service_provider_delete mutate persisted service config with the same path-ID conflict checks as HTTP.
