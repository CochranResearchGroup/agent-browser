...
Notes:
  ...
  - Profile access defaults to shared-local with stable self-declared client IDs. Legacy identity ambiguity migrates to that nonblocking default and appears only on the access health axis. Use service_profile_policy_mutate with expectedRevision plus mode and preset, or an exact targetPolicy, to change access. Administrator, participant, and observer are the human presets. Narrowing an occupied profile fences new admission until the caller clears incompatible users. service_profile_tab_evict accepts only one exact tab from a persisted eviction authorization.
