...
Notes:
  ...
  - Route-bound acquisition automatically completes a matching failed rollback quarantine only after the browser, process identity, session, viewer, controller, handoff, presentation slot, route, display, and route-pool evidence proves inactive. A skipped service_route_pool_repair response reports repaired=false and names the typed evidence that must change before retry.
