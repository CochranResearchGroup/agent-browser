...
the backend-owned profileAllocations view with detail inspection, a grouped
incident browser panel with acknowledgement and resolution actions, and
incident filters for handling state plus crash, disconnect, recovery, timeout,
...
Notes:
  ...
  - POST /api/service/request accepts one intent object with serviceName, agentName, taskName, siteId/loginId, targetServiceId, accountId, url, browserBuild, profile or runtimeProfile hints, profileLeasePolicy, profileLeaseWaitTimeoutMs, action, params, and jobTimeoutMs, then queues the browser command through the same service-owned control path.
...
Notes:
  ...
  - HTTP GET /api/service/access-plan accepts serviceName, agentName, taskName, targetServiceId, siteId, loginId, accountId, url, browserBuild, or their array aliases, then returns the no-launch service-owned profile, policy, provider, challenge, readiness, seedingHandoff, monitorFindings, advisory browserCapabilityEvidence, caller-label warning, and recommendation payload. browserCapabilityEvidence is filtered by the planned browser build and request identity. Preference bindings can set the access-plan browser build recommendation when no explicit request, site policy, or profile browser build has already won; that case reports routingApplied=true with routingScope=access_plan_recommendation. decision.launchPosture includes browserBuild, requiresCdpFree, and cdpAttachmentAllowed so agents and software clients can choose stock Chrome, stealth CDP Chromium, or CDP-free headed posture before opening a browser. Matching active profile_readiness monitors that are due or never checked set monitorFindings.profileReadinessProbeDue and decision.monitorProbeDue, fill decision.monitorRunDue with HTTP, MCP, CLI, and service-client instructions, and recommend run_due_profile_readiness_monitor before relying on retained freshness. Access-plan-backed tab requests marked blockedByManualAction and manualSeedingRequired are refused by the service request client, HTTP POST /api/service/request, and MCP service_request unless allowManualAction is explicitly true. Raw requests carrying monitorRunDueSummary are refused when the summary reports expired, unverified, or missing target freshness evidence unless allowMonitorFreshnessRisk is explicitly true. Copied tab requests marked requiresCdpFree with cdpAttachmentAllowed=false are refused by those same request paths; use action=cdp_free_launch only when process lifecycle and service-state tracking are sufficient.
  - HTTP POST /api/service/browser-capability-registry/<collection>/<id> and MCP service_browser_capability_registry_upsert persist one advisory browser capability registry record through the service worker queue. Path collection and ID are authoritative, and returned upsert records report routingApplied=false because an upsert does not itself route browser work.
