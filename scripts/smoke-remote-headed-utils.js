...
export async function ensureStreamPort(context) {
  const streamStatusResult = await runCli(context, ['--json', '--session', context.session, 'stream', 'status']);
  ...
  if (!stream.data?.enabled) {
    const streamResult = await runCli(context, ['--json', '--session', context.session, 'stream', 'enable']);
