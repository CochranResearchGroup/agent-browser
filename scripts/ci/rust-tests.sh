...
challenge_control_cargo_test=("$script_dir/cargo-safe.sh" test -p agent-browser-challenge-control --manifest-path "$repo_root/Cargo.toml")
...
if [[ -n "$profile" ]]; then
  ...
  challenge_control_cargo_test+=(--profile "$profile")
...
usage() {
  ...
  echo "Compartments: candidate, challenge-control, desktop-services, lease-authority, transport, cli-native, cli-native-actions, cli-native-browser, cli-native-service, cli-native-stream, cli-native-other, cli-workstation, cli-core, cli-integration"
}
...
run_challenge_control() {
  echo "Running Rust compartment: challenge-control"
  "${challenge_control_cargo_test[@]}"
}
...
run_desktop_services() {
  echo "Running Rust compartment: desktop-services"
  "${desktop_services_cargo_test[@]}"
}
...
run_compartment() {
  case "$1" in
    ...
    challenge-control) run_challenge_control ;;
...
run_comprehensive() {
  ...
  (
    ...
    run_and_record challenge-control run_challenge_control "$log_root" || lane_status=1
  ...
  for compartment in cli-native-actions cli-native-service cli-native-other \
    cli-workstation cli-native-browser cli-native-stream cli-core candidate challenge-control desktop-services lease-authority transport cli-integration \
...
case "${1:-}" in
  ...
  --compartment)
    ...
    ;;
  --list-compartments)
    printf '%s\n' candidate challenge-control desktop-services lease-authority transport cli-native cli-native-actions cli-native-browser \
