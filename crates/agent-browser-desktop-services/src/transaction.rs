...
//! verification, and privacy-safe receipt. Platform input and persistence
//! remain injected seams.

use serde::{Deserialize, Serialize};
use sha2::{Digest, Sha256};
use std::collections::BTreeMap;

use crate::coordinator::{DesktopControlCoordinator, DesktopInteractionClaim};

pub const RECIPE_ID: &str = "p110-pointer-keyboard-v1";
pub const FOUNDATION_STRESS_RECIPE_ID: &str = "p110-foundation-stress-v1";
pub const CONTROLLED_X11_RECIPE_ID: &str = "p131-controlled-x11-v1";
pub const TURNSTILE_RECIPE_ID: &str = "cloudflare-turnstile-v1";
pub const HCAPTCHA_RECIPE_ID: &str = "hcaptcha-checkbox-v1";
const TURNSTILE_LOCATOR_ID: &str = "cloudflare-turnstile-v1";
const TURNSTILE_TARGET_CLASS: &str = "cloudflare_turnstile_checkbox";
const HCAPTCHA_LOCATOR_ID: &str = "hcaptcha-checkbox-v1";
const HCAPTCHA_TARGET_CLASS: &str = "hcaptcha_checkbox";
const RECIPE_VERSION: &str = "v1";
pub const FIXED_TEXT: &str = "fixture-ready";
pub const COORDINATE_SPACE: &str = "desktop_physical_pixels";
const FRESHNESS_LIMIT_MS: u64 = 750;

pub fn is_captcha_recipe(recipe_id: &str) -> bool {
    [TURNSTILE_RECIPE_ID, HCAPTCHA_RECIPE_ID].contains(&recipe_id)
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DesktopInteractionRequest {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FoundationStressContext {
    pub prompt_disposition: PromptDisposition,
    pub handoff_reason: Option<String>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DesktopInteractionProviderEvidence {
    pub provider_id: String,
    pub provider_version: String,
...
pub trait ServiceOwnedHandoffRepository {
    fn resolve_ready(
        &mut self,
        browser_id: &str,
        session_name: &str,
...
pub trait DesktopInteractionProvider {
...
pub enum InteractionOperationRecord {
    InProgress {
...
pub trait InteractionOperationLedger {
    fn lookup(
        &mut self,
        caller_id: &str,
        operation_id: &str,
...
pub struct SerializedInteractionOperationLedger {
...
impl SerializedInteractionOperationLedger {
    pub fn from_json(serialized: &str) -> Result<Self, DesktopInteractionError> {
    ...
    pub fn to_json(&self) -> Result<String, DesktopInteractionError> {
...
pub struct InteractionDependencies<'a> {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DesktopInteractionError {
    code: &'static str,
    message: &'static str,
...
impl DesktopInteractionError {
    ...
    pub fn receipt(&self) -> Option<&InteractionReceipt> {
...
pub fn run_desktop_interaction(
    request: DesktopInteractionRequest,
    ...
    if before.observation_status != "matched"
        ...
        || before.selected_target_class.as_deref()
            != Some(match request.recipe_id.as_str() {
                TURNSTILE_RECIPE_ID => TURNSTILE_TARGET_CLASS,
                HCAPTCHA_RECIPE_ID => HCAPTCHA_TARGET_CLASS,
...
pub struct MotionPlan {
    ...
    pub control_points: [PixelPoint; 4],
    pub duration_ms: u64,
    pub hold_ms: u64,
...
pub fn recipe_sha256(recipe_id: &str) -> String {
    digest_text(&format!(
        "{recipe_id}\0{RECIPE_VERSION}\0{}\0{}\0fixed_cubic_bezier_v1",
        match recipe_id {
            TURNSTILE_RECIPE_ID => TURNSTILE_LOCATOR_ID,
            HCAPTCHA_RECIPE_ID => HCAPTCHA_LOCATOR_ID,
            _ => "p110-control-v1",
        },
...
pub fn operation_request_sha256(request: &DesktopInteractionRequest) -> String {
    digest_text(&format!(
        "{}\0{}\0{}\0{}\0{}\0{}\0{}",
        request.browser_id,
...
pub fn provider_effect_key(request: &DesktopInteractionRequest, event_index: usize) -> String {
    digest_text(&format!(
        "{}\0{}\0{}\0{}",
