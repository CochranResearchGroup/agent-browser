//! Provider-free terminal outcome records and deterministic projection.
//!
//! Runtime observation, persistence, transport, and request handling remain
//! in the CLI Adapter. This module owns the durable terminal wire contract.

use serde::{Deserialize, Serialize};
use serde_json::Value;

use crate::{
    ServiceEffectState, ServiceFailureRecourse, ServiceRequestProvenance, ServiceRetryDisposition,
};
...
mod tests {
    ...
    fn outcome_copies_one_failure_and_provenance_without_private_expansion() {
        let provenance = ServiceRequestProvenance {
            request_id: "request-1".into(),
            job_id: "job-1".into(),
            client_subject_id: Some("client:test".into()),
            runtime_lane_id: Some("lane-1".into()),
            action: "open".into(),
            ..ServiceRequestProvenance::default()
        };
        let response = json!({
            "success": false,
            "failure": {
