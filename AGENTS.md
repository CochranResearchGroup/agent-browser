...
serial inside its process, partitions CLI tests into disjoint `native::`,
workstation, and remaining core compartments, and distributes them across two
concurrent lanes through the existing Cargo admission wrapper. The support lane
then runs the CDP crate and CLI integration-test binaries. First-failure logs
remain separate and are printed before the compartment result.
...
`cli-native`, `cli-workstation`, `cli-core`, or `cli-integration`, and
`--list-compartments` for machine-readable discovery. The runner defaults
...
or user-scoped runtime state. Only disjoint compartment processes overlap, with
Cargo admission capped at two. This preserves
in-process isolation while avoiding one monolithic 3,000-test replay. The Rust
