...
For headed access plans, the copied service request includes `params.headless=false` and `params.browserHost`. A copied `remote_headed` request is executable in the daemon: on Linux it starts a hidden Xvfb-backed headed browser when no display is supplied, keeps CDP control available, and records a view stream entry for operator or dashboard surfaces. Set `AGENT_BROWSER_REMOTE_HEADED_DISPLAY` to reuse an existing display, `AGENT_BROWSER_REMOTE_VIEW_URL` to record an external view URL, and `AGENT_BROWSER_REMOTE_VIEW_PROVIDER` when the view provider is not the default `cdp_screencast`. Use `AGENT_BROWSER_REMOTE_VIEW_PROVIDER=rdp_gateway` when that URL points to an HTML5 RDP gateway the dashboard should embed. Run `pnpm test:rdp-gateway-readiness-live` on an operator workstation to verify local `guacd`, `xrdp`, `xfreerdp`, and TCP readiness before expecting an RDP gateway URL to work.
