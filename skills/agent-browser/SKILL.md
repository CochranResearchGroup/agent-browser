...
Use HTTP `GET /api/service/monitors` on the stream port to inspect retained monitor records without scheduling probe work.

Use HTTP `POST /api/service/monitors/<id>`, HTTP `DELETE /api/service/monitors/<id>`, MCP `service_monitor_upsert`, MCP `service_monitor_delete`, `upsertServiceMonitor()`, or `deleteServiceMonitor()` to persist monitor definitions without starting recurring probe execution.
