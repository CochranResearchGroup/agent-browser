...
Access-plan `decision.launchPosture` also reports `viewStreamProvider`, `viewStreamProviderSource`, `controlInputProvider`, and `controlInputProviderSource`. Copied queued service requests carry `params.viewStreamProvider` and `params.controlInputProvider` so agents, software clients, and the dashboard use the service-owned viewing and input posture rather than inferring RDP, noVNC, WebRTC, CDP screencast, or manual desktop input locally. Remote-headed browser records persist the chosen input provider on each `viewStreams` entry as `controlInput`, so operator surfaces can distinguish view-only streams from controllable remote desktop streams.
