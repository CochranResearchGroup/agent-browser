...
The Service dashboard exposes the retained-state cleanup workflow for operators. It queues `service_prune_retained` and `service_repair_retained` through `POST /api/service/request`, shows dry-run candidate counts including orphaned custom profile candidates, and only enables apply after the latest dry-run result is visible for review.
