...
For operator handoff, prefer `agent-browser service profiles <profile-id> seeding-handoff google` or `getServiceProfileSeedingHandoff({ id: "<profile-id>", targetServiceId: "google" })`. That response includes the exact detached `runtime login` command, setup URL, operator steps, and warnings.
