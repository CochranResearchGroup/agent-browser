...
Use `agent-browser service cancel <job-id>` to mark a queued service job cancelled before it dispatches. Running jobs are not interrupted, and terminal jobs are rejected rather than rewritten. Add `--reason <text>` to record why the job was cancelled.
