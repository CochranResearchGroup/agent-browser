...
Recurring route-user password reconciliation must advertise
`routeUserCredentialUpdate.pamBypassed=true` and use the fixed SHA-512 crypt
contract. Do not accept a helper that uses `chpasswd`'s PAM default: it can
invoke `pam_gnome_keyring` and raise an unlock modal independently of Chrome's
`basic_password_store` launch posture.
...
the authenticated dashboard runtime summary for multiplicity, protected,
reclaimable, and unowned RSS, cleanup obligations, retention results, monitor
...
The active ledger keeps an older-reader-compatible failed terminal state and
terminal result. Full candidate-era fields remain in a private terminal-detail
artifact, while the close checkpoint lets current list and inspect output
classify the projection as terminal zero-effect history. A legacy
`closed_zero_effect` enum advertises one exact guarded `close` normalization.
Workstation reconciliation repairs this projection for older terminal records
without rewriting active or operator-recovery transactions.
...
SHA-256 provenance recorded by the installer. Read `runtimeMultiplicity` for
...
daemon endpoint while retaining independent serialized lanes and lane-scoped
close behavior. A workstation install selects the shared host for ordinary
commands. Current binaries reject new legacy per-session daemon launches while
retaining connectivity to an existing legacy socket for explicit ownership
...
For shared, authenticated, persistent, profile-sensitive, or operator-visible
work, use the focused `agent-browser-service` skill when it is installed. It
keeps three readiness axes separate:

1. browser acquisition selects or reuses the profile, browser, session, and
   tab;
2. operator presentation attaches that browser to Guacamole or RDP only when a
   person needs it; and
3. runtime maintenance reports installation, multiplicity, cleanup, and
   retention health.

Start with `service_access_plan` or `agent-browser service access-plan`, then
submit the returned intent through `service_request`. Do not infer that browser
acquisition is blocked merely because every Guacamole route is checked out or
...
or run cleanup to make its request succeed. If only presentation is blocked,
report that axis and either wait or request an explicit route switch. Route
switch may park another browser's presentation while keeping its browser and
profile alive, so it is not an automatic workaround for ordinary automation.

MCP clients can read `agent-browser://operating-guide` for the compact
...
Automation-capable success also returns a valid `serviceTabHandle` at the
top level and under `tab.serviceTabHandle`. Use that handle directly for the
next evaluate, probe, diagnostics, or UI action. Do not issue a redundant
`tab_new`. A generic `tab_new` request with route or display fields is rejected
before queueing; use authenticated `remote_view_open` for route-bound cold
acquisition. When a route-pool entry omits `displayAllocationId`, the derived
...
Agent Browser owns presentation admission, scene resolution, route, display,
window, geometry, capture, verification, release, and cleanup. Never provide
...
`observeServiceDesktopEvidence()`. Request `includeFrame` only when immediate
pixels are necessary. The frame is response-only and stripped from durable
jobs, streams, dashboard records, events, and incidents.
...
durable state. If the exact signed-out Spend entry renders `Page Not Found`, the
owning Authentication Run follows only its fixed recipe login URL before
classification. Other unsupported pages remain operator intervention; do not
redirect or automate them.
...
Development executable replacement uses the two-phase `agent-browser handoff`
protocol. `prepare` keeps the old daemon authoritative and returns a candidate
session. Run `resume --source-session <old>` in that candidate, then run
`finalize` in the old session only after commit and validation. Use `abort` for
a pre-commit failure and candidate `rollback --source-session <old>` for a
post-commit failure. A verified ownerless browser can enter through `resume`
...
agent-browser service profiles <id> repair --capability-file <path> # Plan repair without effects
...
Current responses include additive `statusProjection` provenance and freshness
for host-local observations. Treat reconciled `service_state` and a present
...
helpers, Service State, and the dashboard. Use
it to inspect the shared multiplicity state, active convergence window,
lifecycle and cleanup-obligation counts, reconciliation freshness, protected
versus reclaimable pressure, profile and generation retention effects, and a
blocking incident without inspecting processes or private files. Steady state
is one dashboard, one runtime host, zero legacy daemons, and one selected
generation. A transaction-bound `convergence_window` is temporary and must
return to steady state. Never treat a dry-run candidate, review-gated profile,
persistent profile, or ambiguous resource as authorization to delete it.

Use `agent-browser service state validate --path <absolute-path> --json` when an
installed candidate must prove it accepts exact Service State bytes before
those bytes become runtime state. It runs the durable store's bounded-stack
parser and cross-record invariant checker and returns `stateSha256`,
`parserIdentitySha256`, `accepted`, and an `accepted` or `error`
classification. The parser identity hashes the running executable. This
command performs no writes, does not consult or recover the default Service
State store, and never starts a service or browser.

For postmortems, use `GET /api/service/failures?limit=100` or inspect
`~/.agent-browser/service/failure-journal.jsonl`. This append-only journal is
independent of the bounded Service event list and captures failed launches,
terminal service actions, Guacamole loads, unusable handoffs, non-streaming
CDP feeds, and failed dashboard actions. Authenticated client-only observations
go to `POST /api/service/failure-observation`. Never submit a raw handoff URL,
raw handoff ID, credential, page content, header, bearer value, or query string.
Pending records live in `~/.agent-browser/service/failure-journal.pending/`.
Unix custody syncs the file and directory before returning; daemon startup
replays pending occurrences without resubmitting failed requests. Queue or
...
Use `agent-browser service leases`, HTTP `GET /api/service/profile-leases`, MCP `agent-browser://profile-leases`, or `getServiceProfileLeases()` to inspect first-class profile lease authority without launching a browser. Each record reports the authenticated principal, exact owner generation, subordinate session and tab work, blocking identity axes, authorized actions, typed continuity recourse, and doctor findings. A current canonical profile claim is the sole operational row for that profile. It reports `state=active` and `recourse=continue_with_active_claim`; retained sessions and owner bindings remain visible context but cannot add doctor blockers. The logical claim does not prove a live browser process. Use `service leases doctor`, `service leases <id> inspect`, or `service leases <id> explain` for focused diagnosis. Register a service principal with `service leases register --principal-id <id> --profile-id <id> --capability-out <absolute-private-path>` before or after first launch. Rejoin, renew, and release require the exact current `--revision` and read the capability from `--capability-file`; never put the raw capability on the CLI command line. When a pre-registered capability later has one exact uncontested ready owner session, or a registered lease is blocked by `unproven_session_authority`, with or without `owner_generation_or_binding_mismatch`, and lists `rejoin` as authorized, run `service leases <id> rejoin --revision <revision> --capability-file <path> --expires-at <rfc3339>`. Rejoin may establish the missing binding or compare-and-swap the same principal, profile, and capability binding to a newer current ready owner generation, then binds only the unique exact live session and its same-browser active tabs. A pre-registered matching principal's first authenticated `tab_new` request receives a stable principal-owned daemon session derived from public principal and profile identity. The broker prepares that route before relay and attaches an internal route authorization receipt. The daemon revalidates the capability ID and revision, profile, planned session, current owner generation when present, and competing live authority before profile selection, so an unrelated transport default session cannot capture the request and a valid prelaunch session is not mistaken for an unproven retained identity. Raw capability material is never part of the route or receipt. A capability bound to one profile cannot cold-launch or reuse a different profile, including when that target has no retained session or lease. If `owner_generation_or_binding_mismatch` is the only blocker and no subordinate session work remains, use `service leases <id> reconcile plan` with the current revision and capability. An effect-capable plan contains one `refresh_principal_owner_binding` transition only when the exact ready profile, browser, process, and session route agree; apply rechecks those identities and the boot epoch. After an unexpected browser exit, a non-expired registered session work lease preserves exact profile selection until replay establishes the next owner. Labels, expired work, and legacy observations do not authorize replay. Ambiguous or foreign work stays blocked. HTTP mutations require `Authorization: Bearer <capability>`. MCP mutation tools accept `profileCapability` only as an ephemeral tool argument; never log or persist it. Legacy and unbound identity rows remain observation-only. Rows backed only by released or expired legacy sessions use `state=historical`, expose read actions only, have no blocking identity axes, and do not make lease doctor unhealthy. Current nonterminal legacy session or owner evidence requires reconciliation only when no canonical claim exists. If a current row advertises `profile_acquire`, use the capability-bound profile acquisition command or the dashboard Acquire profile action; do not try to force a lease mutation. Dashboard Reconcile plan maps only to the backend `reconcile_plan` action. Use `findServiceProfileLease()` to resolve an exact lease ID from a collection response.
...
If the registered private capability file is lost, run `service leases capability-status --principal-id <id> --profile-id <id>`. Rotate only when it reports one active grant, `rotationAllowed=true`, and no blockers. Pass its public `capabilityId` and `registryRevision` to `service leases rotate-capability` with a new absolute `--capability-out` path. Rotation refuses active claims and subordinate work, revokes only the exact compare-and-swap grant, writes a new `0600` file, and never recovers or prints the old secret. This is an operator-local CLI recovery surface; do not improvise state edits or use HTTP or MCP.
...
Profile access defaults to `shared-local`. Supply a stable self-declared `clientSubjectId` for ordinary local sharing. If no durable profile is selected and site policy does not require one, accept the broker's deterministic disposable `managed-ephemeral-*` profile and executable launch request. Keep the same labels or explicit subject on later requests so the broker can continue a consistent shared-local session without strict runtime-owner proof. Ambiguous legacy identity migrates to the same nonblocking policy and remains visible on the access health axis. Use `service_profile_policy_mutate` with `profileId`, `expectedRevision`, and either `mode` plus an `administrator`, `participant`, or `observer` preset, or an exact `targetPolicy`. Narrowing occupied access establishes a drain before commit. Successfully closed tabs remain as history and no longer block drain completion; disconnected or otherwise nonterminal tabs still count. The caller requesting narrower access is responsible for clearing incompatible users. Use `service_profile_tab_evict` only with the exact `authorizationId` and `tabId` from a persisted eviction plan. Do not turn access ambiguity into a runtime or installation blocker.
...
If a registered profile needs interactive authentication, queue `service_profile_manual_seeding_acquire` with `profileId` and `targetServiceId`. It reserves the exact registered profile and a ready keeper slot in SQLite and returns an opaque `/remote-view/<handoff-id>` URL only after current presentation proof. Give the operator only that URL. The headed browser has no CDP attachment, and a replay reuses its exact live PID and handoff. Handoff resolution rechecks process and route visibility. If launch identity is uncertain, do not retry while its recorded PID exists. After that PID is absent, acquire returns `retryRequiresNewOperation: true` and a new request ID is required for another launch. When manual work is complete, queue `service_profile_manual_seeding_close` with the returned `profileId`, `targetServiceId`, `handoffId`, and `pid`. Close terminates only the verified process instance and closes the handoff; the provider-owned keeper remains ready for reuse. Then follow the returned attachable relaunch and separate authentication-probe instructions. Never treat visible browser content as proof of authentication.
...
`service status` exposes replayable recovery progress through the redacted `crashRegenerationTransactions` array. Use its stable identity fields, current phase, readiness, and `recourse` value for operator guidance. Never infer host PID, socket, display, viewer-session, or provider-generation authority from that public projection; those ephemeral details remain private transaction evidence.
...
Use `service profiles <profile-id> repair --capability-file <path>` to plan without effects. Apply only the reviewed sealed plan with `repair --apply --plan <path> --session-name <route> --capability-file <path>`. The apply path rechecks all bound profile, owner, process, route, reference, and lock evidence, attempts one repair, and replays its receipt without another effect. Changed, live, foreign, or ambiguous evidence is a stop condition. HTTP repair plan/apply routes, MCP `service_profile_repair_plan` and `service_profile_repair_apply`, and the generated repair helpers have matching semantics.

Use `service profiles <profile-id> reset --scope runtime --capability-file <path>` only for an exact proven inert service-owned lane. Runtime reset preserves the profile directory, authentication evidence, and peer lanes. Use `--scope authentication --target-service-id <id>` to remove only that target's retained authentication evidence. It never erases browser cookies and returns a manual-seeding handoff. Full profile-data reset is unavailable until backup restore and rollback are supported. Both supported scopes require an unchanged sealed plan before apply; HTTP, MCP, and generated reset helpers expose the same contract.
...
Use `agent-browser service access-plan` when a shell operator or CLI-driven agent needs the same no-launch broker recommendation as HTTP `GET /api/service/access-plan`, MCP `service_access_plan`, or `agent-browser://access-plan`. Pass `--service-name`, `--agent-name`, `--task-name`, `--session-name`, `--login-id`, `--target-service-id`, `--account-id`, `--url`, `--browser-build`, `--browser-host`, `--view-stream-provider`, `--control-input-provider`, or `--display-isolation` when known. Use `--session-name <lane>` for an existing service session or the exact `replacementSessionName` advertised for a cleanup-satisfied terminal owner. A unique live mapping fills both reuse hints; the terminal replacement route is copied into a fresh-launch request automatically. A missing, ambiguous, incompatible, or non-authorized replacement session makes the planned request unavailable instead of falling back to `session:default`. Text output includes selected profile, manual-seeding posture, monitor freshness recommendation, service request availability, and compact `browser_build_summary` fields so the CLI path has the same routing audit information as software clients. JSON responses include `decision.profileReuse`, which tells clients and dashboard launcher eligibility rows whether to reuse an existing compatible browser, wait for the selected profile lease, launch a new browser, or stop at an explicit-session blocker. A healthy retained browser linked only to inactive sessions reports `rejoin_profile_lease` and `expired_session_recovery_required`; inspect and rejoin the exact profile lease, then request a fresh plan. `compatibleLiveBrowserCount` counts operation-compatible browsers; `sameProfileLiveBrowserCount` is the broader health observation. When the recommendation is `reuse_existing_browser`, copy the planned `decision.serviceRequest.request` with its top-level `browserId` and `sessionName` route hints so `POST /api/service/request`, MCP `service_request`, and generated client helpers use the existing daemon lane. When `requiredAction` is `supersede_terminal_owner`, copy the same planned request with its exact replacement session route. Never reconstruct or override those route hints. Nonterminal lifecycle and owner rows remain diagnostic evidence and cannot block cold launch or supply route identity without a canonical current claim or fresh exact physical collision. Direct launches that select a profile already backed by a live retained browser are rejected unless the request uses those route hints or sets `allowDuplicateProfileLane: true` for reviewed isolation or throwaway browser behavior.

When a service request fails, preserve its legacy `error` string and consume
the versioned `failure` object with `getServiceFailureRecourse()` or
`requireServiceRequestSuccess()`. For `service_state_lock_timeout`, an
`effectState` of `effect_uncertain` means the browser action may have completed
before terminal persistence failed. Inspect the exact job, trace, and current
resource state before another mutation. For
`runtime_lifecycle_existing_owner_requires_explicit_transition`, refresh the
access plan once. Reuse is permitted only when that current plan provides the
exact compatible `browserId` and `sessionName`; otherwise inspect the offered
Recovery Plan or stop on the typed blocker. Never blind-retry or launch a
duplicate profile lane.

Read `serviceStateLockDiagnostics` from a current `service status` response when
diagnosing contention. Its bounded process-local `active`, `recent`, and
`counters` fields distinguish a lock that is still held from a completed
timeout and expose wait and hold durations without state paths or request
payloads. This telemetry is diagnostic only and is never persisted into
Service State. It does not change the failure object's retry disposition.

An exclusive file-lock holder also writes a bounded sidecar beside Service
State. A `service_state_lock_timeout` error may append validated
`holder_status`, PID plus process-start identity, build generation, operation,
mode, phase, monotonic wait and elapsed-hold timing, acquisition wall time, and
available coarse state counts. Treat stale, corrupt, missing, or
unobservable holder evidence as unknown. Never use this sidecar to force an
unlock, kill a process, take over a profile, or authorize a browser retry. The
repository may replay one pure in-memory state mutation after a stale revision;
this does not replay browser, tab, route, provider, or tenant effects.
...
For static route-user setup beyond the legacy pair, set `AGENT_BROWSER_RDP_ROUTE_USER_POOL_JSON` to a JSON array of `id`, `connectionName`, and `routeUser` entries; `legacyConnectionName` is optional. Setup generates missing passwords into the user-scoped Guacamole secret file and preserves every configured route. When the array is absent, the legacy two-route environment is accepted only through the compatibility adapter. Treat the full `AGENT_BROWSER_RDP_ROUTE_POOL_JSON` emitted by inspection as canonical; the first two display variables remain compatibility output.

Read `presentationCapacity` from service status before requesting operator-visible desktop work. It reports slot states, admitted and hard limits, human and recovery reserves, queued demand, and redacted binding warnings without launching a browser or opening a route. Logical CDP-only browsers do not consume presentation slots. Human controllers block automated staging; passive viewers permit capture only when the exact scene is already ready.

Read `desktopEvidencePolicy` before choosing a capture surface. Use CDP for DOM,
accessibility, viewport, canvas, and page pixels. Use a desktop evidence episode
only when the expected evidence lives outside page CDP, such as browser chrome,
extension UI, a password-manager or passkey prompt, a native dialog, an OS
window, or stacking and occlusion. A generic CDP failure does not authorize
...
Use MCP `service_request` when a service wants to queue one intent-based browser action with caller context, site or login hints, explicit `profile` or `runtimeProfile` hints, optional top-level `browserId` or `sessionName` reuse route hints, `action`, `params`, and `jobTimeoutMs` in a single request object. This is the service-oriented equivalent of `browser_command` for clients that want agent-browser to own profile selection, queueing, and trace metadata deterministically. Top-level `browserId` and `sessionName` route ordinary commands to an existing retained daemon lane. A complete pair is validated against the current access plan and then attaches that exact compatible retained browser; a contradictory pair is rejected before browser effects. Action-specific `params.browserId` and `params.sessionName` remain scoped to actions such as `view_takeover`. Use `allowDuplicateProfileLane: true` only for reviewed isolation or throwaway browser behavior.

A cold shared-local access plan supplies a stable `shared-profile-*` session derived from its admitted self-declared subject and profile. Copy that request so the runtime host's ambient lane cannot contribute unrelated identity.

HTTP and MCP request-normalization failures include structured recourse. For an access-plan route conflict, preserve the exact failure code, treat it as no effect, refresh the access plan, and submit its exact route. Do not blindly retry or launch a duplicate Profile lane.
...
Dashboard viewer reconnect derives lease authority from the browser's service-owned `rdp_gateway` stream rather than a selected CDP presentation stream. Never pass a synthetic `daemon:<session>` stream identifier as a remote-view route ID. Remote-view lifecycle requests have a bounded extended dashboard proxy budget, and reconnect failures retain their typed backend code for diagnosis.
...
During an explicitly reviewed emergency lease-repair window, the service daemon
...
`profileLeaseFailOpen` metadata and a warning; treat that browser as
unauthenticated unless the task establishes identity separately.

If the operator explicitly accepts cross-client ownership loss during the same
repair window, `AGENT_BROWSER_PROFILE_LEASE_MODE=unsafe_claim_any` permits any
attributable service request to select an exact `sessionName` and profile across
principal, capability-owner, owner-generation, and profile-lease conflicts.
Prefer an existing named session; the broker adopts its sole retained browser
when known. Require `profileLeaseUnsafeClaim.applied=true` in evidence and remove
the setting immediately after the repair. This mode does not bypass dashboard
login, workstation upgrade admission, action policy or confirmation, or
viewer/controller authority.
...
Read structured `targetReadiness` rows before asking the operator to seed a profile. A stealth-CDP Google or Gmail profile reports attachable readiness unless a site policy explicitly requires CDP-free operation. Required CDP-free rows report `seedingMode: "detached_headed_no_cdp"`, `cdpAttachmentAllowedDuringSeeding: false`, `preferredKeyring: "basic_password_store"`, and `setupScopes` such as `signin`, `chrome_sync`, `passkeys`, and `browser_plugins`. Do not reduce either posture to a generic "use a runtime profile" instruction.
...
Effect-capable service requests need an accountable principal before queueing.
Supply nonempty `serviceName`, `agentName`, and `taskName` labels for HTTP and
generated-client calls. Authenticated dashboard calls may use the dashboard
identity, and local MCP stdio calls receive a local process principal plus
request ID. Unauthenticated unlabeled HTTP requests fail before relay.
Collection reads stay label-optional and no-launch; run
`pnpm test:service-collections-no-launch` after changing these boundaries.
...
Resource kinds use executable names rather than installation directory names.
Managed Chrome under `.agent-browser` remains a browser; that label does not
establish ownership or authorize cleanup of an unproven process.
Linux resource inventory can report `verified_retained_browser_descendant` when
current-boot owner and lifecycle evidence bind the exact root process and a
consistent same-executable ancestry chain. This supplies resource protection
...
preserve a newer re-seeding record or concurrent deletion.

Use `agent-browser service resources` before manual process cleanup when browser hosts are consuming too many resources. It builds a no-launch process inventory for agent-browser daemons, browser processes, and remote display processes, then correlates each row with retained service browser IDs, profile IDs, session IDs, display allocation IDs, profile paths, and CDP ports when possible. Its `runtimeLanes` projection and summary counts expose current owner generations and cleanup obligations. Package-launched and managed-attached Chrome lanes register canonical profile, exact process, CDP endpoint, and target evidence before ready state; transfer, retained detach, recovery, and close must use the same lifecycle authority. A cleanup obligation becomes satisfied only after exact process exit and profile-lock release. The classifier protects the dashboard service main PID, retained active browser PIDs, and named or persistent managed profile data. An exact lifecycle-owned `closing` process tree can still become a GC candidate without making its named or persistent profile data deletable. Resource warnings include `duplicate_live_browsers_for_profile` and `duplicate_active_profile_leases`; `agent-browser install doctor` reports those as `service_duplicate_profile_pressure`. Use `agent-browser service gc --dry-run` to group conservative cleanup candidates and receive a short-lived review token. Use `agent-browser service gc --apply --review-token <token>` only after reviewing that exact dry-run. Apply mode fences root PID, start token, executable, process group, owner generation, launch identity, and profile identity before signaling the full group. It sends SIGTERM first, uses SIGKILL only for a still-matching tree, and removes only a stale `SingletonLock` after the exact process group exits. Use `--force-without-review` only for explicit operator override. The optional read-only timer is installed with `scripts/install-resource-monitor-user-timer.sh`, removed with `scripts/remove-resource-monitor-user-timer.sh`, writes only aggregate status to `~/.agent-browser/service/resource-monitor-summary.json`, and can be read with `agent-browser service resources --monitor-summary`.

Resource policy uses bounded environment inputs. `AGENT_BROWSER_RESOURCE_INACTIVITY_MIN_SECONDS` defaults to 300 seconds with a 60 through 86400 range and controls abandoned-lane eligibility. `AGENT_BROWSER_RESOURCE_PER_BROWSER_MAX_RSS_BYTES` defaults to 4294967296 with a 67108864 through 68719476736 range; `AGENT_BROWSER_RESOURCE_PER_BROWSER_MAX_DESCENDANTS` defaults to 64 with a 1 through 4096 range; `AGENT_BROWSER_RESOURCE_PER_BROWSER_MAX_TABS` defaults to 128 with a 1 through 16384 range; `AGENT_BROWSER_RESOURCE_WORKSTATION_MAX_LANES` defaults to 16 with a 1 through 4096 range; `AGENT_BROWSER_RESOURCE_WORKSTATION_MAX_PROCESSES` defaults to 256 with a 1 through 65536 range; and `AGENT_BROWSER_RESOURCE_WORKSTATION_MAX_RSS_BYTES` defaults to 68719476736 with a 67108864 through 1099511627776 range. Those six limits produce warnings only and never authorize cleanup. Do not lower thresholds to manufacture effect authority; retirement still requires the reviewed exact candidate and fresh pre-effect revalidation.
...
Service control jobs default to a 900000 ms worker deadline. Set `service.jobTimeoutMs`, pass `--service-job-timeout <ms>`, or set `AGENT_BROWSER_SERVICE_JOB_TIMEOUT_MS` to choose another deadline. A zero or absent configured value selects the built-in default, so every dispatched job has a recovery horizon. Service reconciliation marks stranded `running` records `timed_out` after their persisted deadline without retrying the original effect; legacy records without timeout metadata use the same 900000 ms recovery horizon. For one ordinary CLI command, pass `--job-timeout-ms <ms>` so the daemon cancels that command and releases its serialized queue before any longer caller-side subprocess deadline expires. If the caller disconnects first, the daemon releases only that transport connection's profile custody while the accepted job continues to a retained terminal state. Inspect that job before taking another action; do not replay it automatically.

Prepared Service State mutations are serialized across runtime-host processes
from authoritative load through durable commit. Do not add caller-local
`service_state_stale_revision` retry loops. If a reviewed command needs longer
to enter this serialized lane, pass
`--service-state-lock-timeout-ms <positive-ms>`. The override is capped at five
minutes and changes only the contention wait. It does not overwrite stale
state, repeat an uncertain browser effect, or bypass lifecycle, profile, route,
presentation, or effect authority. For Service jobs, confirm the retained
result records the requested `serviceStateLockTimeoutMs` value.
...
`dashboard start` keeps the requested port as stable ingress and starts the
generation-specific backend on the next port. The ingress retains the prior
selected backend as a draining fallback and changes selection only after a
shadow candidate passes runtime-manifest and authenticated operator-journey
proof. Use `agent-browser dashboard ingress status` to inspect
`dashboardIngressReady`, `operatorJourneyReady`, selected, candidate,
fallback, and presentation-receipt state. Coordinators use `dashboard ingress
stage`, `commit`, and `rollback` with exact `--expected-revision` values. Do
not synthesize presentation evidence or commit before the authenticated
journey is complete. `AGENT_BROWSER_DASHBOARD_INGRESS_STATE` overrides the
private registry path only for isolated runtimes. Also set
...
`replacementEligible`, `requiredAction`, and the exact
`replacementBrowserId` and `replacementSessionName` for an eligible terminal
owner. Use the copied request, which carries that replacement session; follow a
reconcile or inspection requirement instead of retrying a vetoed launch.
...
ingress and restarts only the backend; restarting the single dashboard unit is
the compatibility path for older unsplit installs.

After login, the dashboard polls authenticated `/api/runtime/health` every ten
seconds. The `agent-browser.runtime-health.v1` response detects executable,
package, and stream drift and renders `Runtime status out of sync` without
restarting active sessions.
...
`occurrenceId` for their authenticated journal record and, when known, a
`terminalOccurrenceId` referencing the owner event. They retain `retrySafe=false`.
...
reconciliation before another attempt. Connection readiness does not establish
...
Reattaching to the same verified browser preserves its original host, display
allocation, launch metadata and recorded profile path. A CDP attachment is a
connection to that browser, not a replacement launch. Preservation requires
...
are preserved; a missing inventory for a staged provider remains an error.
...
The dashboard sends operator focus through its authenticated request path.
