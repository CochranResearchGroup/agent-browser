...
Access-plan `decision.launchPosture` also reports `viewStreamProvider`, `viewStreamProviderSource`, `controlInputProvider`, and `controlInputProviderSource`. Copied queued service requests carry `params.viewStreamProvider` and `params.controlInputProvider` so agents, software clients, and the dashboard use the service-owned viewing and input posture rather than inferring RDP, noVNC, WebRTC, CDP screencast, or manual desktop input locally. Remote-headed browser records persist the chosen input provider on each `viewStreams` entry as `controlInput`, so operator surfaces can distinguish view-only streams from controllable remote desktop streams. They also expose `displayIsolation` and `displayName` when the service can tell whether the browser uses a private virtual display, an explicitly shared display, or the daemon's ambient `DISPLAY`. The dashboard enables View only for embeddable streams and Control only when service metadata includes an input provider.

For headed access plans, the copied service request includes `params.headless=false` and `params.browserHost`. A copied `remote_headed` request is executable in the daemon: on Linux it starts a hidden Xvfb-backed headed browser when no display is supplied, keeps CDP control available, and records a view stream entry for operator or dashboard surfaces. Treat one private virtual display per browser process as the default isolation boundary. That keeps focus, window geometry, input injection, streaming, recording, crash recovery, and repair scoped to the selected browser. Set `AGENT_BROWSER_REMOTE_HEADED_DISPLAY` only when an operator intentionally wants to reuse an existing shared display for trusted low-contention work. Set `AGENT_BROWSER_REMOTE_VIEW_URL` to record an external view URL, and set `AGENT_BROWSER_REMOTE_VIEW_PROVIDER` when the view provider is not the default `cdp_screencast`. Use `AGENT_BROWSER_REMOTE_VIEW_PROVIDER=rdp_gateway` when that URL points to an HTML5 RDP gateway the dashboard should embed. The Service inspector shows selected-browser view readiness, control readiness, display isolation, provider, input, and gateway URL from the retained stream metadata. Its selected-browser `Open remote control` action uses the same queued `view_focus` path as browser rows before opening the embedded stream. Run `pnpm test:rdp-gateway-readiness-live` on an operator workstation to verify local `guacd`, `xrdp`, `xfreerdp`, and TCP readiness before expecting an RDP gateway URL to work. Run `pnpm test:service-remote-view-control-live` when changing remote-headed launch, retained view-stream records, or dashboard View and Control gating; it launches an isolated remote-headed browser, verifies stream and input metadata, runs queued `view_focus`, and closes the browser through the service-owned path. Run `pnpm test:service-dashboard-remote-control-ui-live` when changing the selected-browser inspector or embedded dialog; it drives the real dashboard UI, clicks the right-pane remote-control action, verifies the iframe URL, and confirms the dashboard queued `view_focus`.
...
Retained service jobs preserve target identity hints for profile debugging. Singular `targetServiceId`, `siteId`, and `loginId` fields keep exact singular request values when present, and `targetServiceIds` stores the normalized target-service, site, and login identity set used for profile selection. Jobs also include `controlPlaneMode` (`cdp`, `cdp_free`, or `service`) and `lifecycleOnly` so callers can distinguish CDP-free launch and service lifecycle jobs from CDP-backed tab interactions. Prefer these fields in `service jobs`, HTTP jobs, MCP `agent-browser://jobs`, and service trace output instead of reconstructing identity or attachment posture from the original command payload.
...
The dashboard runs independently of browser sessions on port 4848 (configurable with `--port`). All sessions automatically stream to the dashboard. On Linux hosts where ingress depends on a stable dashboard listener, run `bash scripts/install-dashboard-user-service.sh` from the repo to install and start a user-scoped `agent-browser-dashboard.service`; set `AGENT_BROWSER_DASHBOARD_PORT` first when the listener should use a non-default port. Sessions can also be created from the dashboard UI with local engines or cloud providers. Use the Service view to inspect worker/browser health, set a remembered operator identity for incident audit metadata, add optional operator notes when acknowledging or resolving incidents, inspect prominent service-sourced incident severity, escalation, recommended action displays, and remedy summary groups, inspect the service-owned incident history timeline with local fallback, load combined service, agent, task, browser, profile, session, and time-window traces from `/api/service/trace`, including ownership summary cards, naming warnings, and profile lease wait cards from the shared trace payload, view a browser-health transition timeline for crash/recovery visibility, inspect the backend-owned `profileAllocations` view and detail dialog for holder sessions, waiting jobs, conflicts, lease state, and recommended actions refreshed from `GET /api/service/profiles/<id>/allocation`; run `pnpm test:dashboard-profile-allocation` when changing that detail lookup. Use a grouped incident browser panel with handling-state filters plus acknowledge and resolve actions, inspect incident filtering for crash/disconnect/recovery and timed-out or cancelled jobs, check reconciliation status, review managed entity counts, inspect recent service jobs with naming warnings, select managed browser, profile allocation, incident, session, tab, and job rows into the desktop right-pane inspector, acknowledge or resolve selected incidents from the right pane, cancel selected queued or running jobs from the right pane, open embedded remote-view streams for browsers or service tabs with a fullscreen toggle, rely on queued `view_focus` to bring the selected tab forward and request native-window maximize before inspection, inspect event details, and run reconciliation from the UI.
