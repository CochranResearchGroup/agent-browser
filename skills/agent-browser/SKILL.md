...
Use `service recovery plan` only with the capability for the exact profile. Plan generation is zero-effect. The first effect-capable class is an exact terminal owner with satisfied cleanup, proven process exit, released profile lock, and no active foreign lease. Apply reads the sealed plan from an absolute path, targets only its sealed daemon route, rechecks every bound identity, retries acquisition once, and persists a receipt. Use `service recovery status` to read that receipt. HTTP, MCP, and generated clients expose matching plan, apply, and status operations. Never reinterpret `existing_session_profile_identity_unproven` as terminal-owner evidence; it requires separate principal/profile reconciliation and must not launch a duplicate browser.
