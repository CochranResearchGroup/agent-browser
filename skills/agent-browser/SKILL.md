...
The MCP `agent-browser://monitors` resource returns retained service monitor records for heartbeat, tab, site-policy, and login freshness probes without launching Chrome. For operator triage, `agent-browser service monitors --summary --failed`, `GET /api/service/monitors?summary=true&failed=true`, and `getServiceMonitors({ summary: true, failedOnly: true })` return compact failure counts, repeated-failure counts, never-checked counts, and matching monitor IDs. Add `--state faulted` or `state: "faulted"` to focus on faulted monitors. MCP keeps the unfiltered resource shape for stable agent reads.
...
Use HTTP `POST /api/service/monitors/<id>`, HTTP `DELETE /api/service/monitors/<id>`, MCP `service_monitor_upsert`, MCP `service_monitor_delete`, `upsertServiceMonitor()`, or `deleteServiceMonitor()` to persist monitor definitions for the scheduler.
