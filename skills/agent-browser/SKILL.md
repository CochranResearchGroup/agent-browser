...
If a bounded direct CLI launch is appropriate, first verify that `agent-browser service status` reports `launchConfig.defaultBrowserBuild` as `stealthcdp_chromium` and `launchConfig.stealthCdpChromiumReady` as ready, then preserve the remote-control posture explicitly:

```bash
DISPLAY="${AGENT_BROWSER_REMOTE_HEADED_DISPLAY:-:10}" \
agent-browser \
  --browser-host remote_headed \
  --view-stream-provider rdp_gateway \
  --control-input-provider manual_attached_desktop \
...
Prefer the dashboard or queued service request when available because it records the Guacamole view URL, display, input provider, profile lease, session, and browser health as service state. Use local headless or direct local headed sessions only for throwaway no-login smokes, non-visual extraction, or when an operator explicitly asks for a local browser. Use `cdp_free_headed` only when an access plan or site policy says CDP attachment is not acceptable.
...
The Service dashboard exposes the retained-state cleanup workflow for operators. It queues `service_prune_retained` and `service_repair_retained` through `POST /api/service/request`, shows dry-run candidate counts, and only enables apply after the latest dry-run result is visible for review.
