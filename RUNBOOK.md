...
admits Plan 0187 W6 on `challenge/p197-consumer-integration` from clean
`origin/main@faee8887`. Authentication and navigation are the first two
consumers. They will use one provider-neutral challenge admission contract that
binds a completed receipt to the effective site-policy digest, downstream
intent, principal, and exact current tab before consumer execution. P190 is
retained as the primary writer for its candidate, installer, CLI-help, README,
command-docs, and agent-skill surfaces. P197 performs provider-free source and
