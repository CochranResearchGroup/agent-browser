...
`service leases` exposes first-class profile lease authority without launching a browser. HTTP `GET /api/service/profile-leases`, MCP `agent-browser://profile-leases`, and generated helpers `getServiceProfileLeases()` and `findServiceProfileLease()` return the same collection. Each record names the authenticated principal, exact owner generation, subordinate session and tab work, blocking identity axes, authorized actions, typed continuity recourse, and doctor findings. Legacy retained evidence remains observation-only and caller labels cannot grant profile authority.
