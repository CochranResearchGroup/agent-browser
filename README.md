...
`docs/dev/contracts/service-request*.json`, and run
...
with declarations generated from the service job, event, incident, incident
activity, and trace schemas.

See `examples/service-client/` for a copyable workflow that sends a service
request and then reads the matching trace. Run
`pnpm test:service-client-example` to validate the example in dry-run mode.
Run `pnpm test:service-client-example-live` to validate the same example
against a real isolated daemon and browser session.
