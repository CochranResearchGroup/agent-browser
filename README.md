...
A registered profile that needs interactive sign-in can use queued service request action `service_profile_manual_seeding_acquire` with `profileId` and `targetServiceId`. It reserves the exact registered profile in SQLite, opens one keeper-bound headed browser without CDP, and returns only an opaque `/remote-view/<handoff-id>` operator handoff after presentation proof. Replays reuse the same live process and handoff. Resolving that URL checks current process and route visibility. If launch identity was uncertain, the profile stays reserved while the recorded PID exists; once that PID is observed absent, acquire terminalizes the old operation and returns `retryRequiresNewOperation: true` without launching again. If a previously ready browser has exited, acquire or handoff resolution closes its exact handoff and returns the same retry marker. Submit a new request ID to retry. After the operator finishes, call `service_profile_manual_seeding_close` with the exact returned `profileId`, `targetServiceId`, `handoffId`, and `pid`. Close verifies and stops only that process, closes its handoff, retains the provider-owned keeper route, and returns separate attachable relaunch and authentication-probe next actions. Visibility is never authentication evidence.
