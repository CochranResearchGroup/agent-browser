...
The HTTP API loads the same persisted and configured service state as the CLI before relaying state-changing requests to the daemon. The collection endpoints return the same compact arrays as the MCP resources: `profiles`, `sessions`, `browsers`, `tabs`, `sitePolicies`, `providers`, and `challenges`, each with a `count` field.
