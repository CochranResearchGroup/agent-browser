...
same migration only after candidate preflight and a stable runtime census, then
records an exact backup locator, restore procedure, before and after digests,
summary, contamination report, and terminal migration receipt in the private
transaction artifacts. The default migration never deletes profiles, browsers,
sessions, displays, routes, leases, handoffs, principals, capabilities, runtime
owners, or lifecycle identities. Unknown successor fields and action types are
...
input-method changes. Embedded viewers suppress both declarative autofocus and
the text-input controller's initialization focus call;
...
`handoff prepare` writes a private proposal with process, profile, endpoint,
target, and owner-generation evidence. The old daemon stays effect-capable.
Run `handoff resume --source-session <old>` in the candidate session to attach
observation-only, revalidate the retained browser, and atomically commit the
candidate owner. Then run `handoff finalize` in the old session to relinquish
its browser connection and stop it. Use `handoff abort` in the old session for
a pre-commit failure. Use `handoff rollback --source-session <old>` in the
candidate session after commit; it restores the old owner at a new generation
and preserves the browser. A verified ownerless browser can use the same resume
...
Service mode is the persistent control plane for long-lived automation. It keeps profile, session, browser, tab, monitor, job, incident, event, site-policy, provider, and challenge state aligned across CLI commands, the HTTP API, MCP resources/tools, and the dashboard. Agents should include `serviceName`, `agentName`, and `taskName` when available so multi-service work remains traceable. The normal service request is identity-first: ask for a tab or browser action, target site or login identity, and the owning service, agent, and task. agent-browser selects or reuses the managed profile and browser, serializes CDP work through the queue, and records the state needed for debugging. Service profile records and profile allocation rows include `targetReadiness`, a no-launch readiness view for target services. Built-in Google and Gmail targets without authenticated evidence use headed stealth Chromium and report attachable unknown readiness; only an effective CDP-free policy reports `needs_manual_seeding` and recommends detached `runtime login`. Once a managed profile lists the target in `authenticatedServiceIds`, readiness changes to `seeded_unknown_freshness` and access-plan no longer treats first-login seeding as a required manual action. Access-plan responses also include `monitorFindings` and `decision.monitorAttentionRequired` when an active `profile_readiness` monitor is faulted for the requested target identity. When a matching active `profile_readiness` monitor is due or never checked, access-plan sets `monitorFindings.profileReadinessProbeDue`, fills `decision.monitorRunDue`, and recommends `run_due_profile_readiness_monitor` before the caller trusts the profile. Use an explicit managed runtime profile when you know where the needed login state lives; use `--profile <path>` only when bringing an external profile is part of the contract.
...
cleanup states, terminal evidence, `replacementEligible`, the required action,
and the exact `replacementBrowserId` and `replacementSessionName` authorized
for a cleanup-satisfied terminal owner. The copied launch request carries that
session route automatically so `supersede_terminal_owner` is executable
without falling back to a colliding default lane.
...
1–8 lowercase letters or digits, starting with a letter. Its launcher, service
units, state, containers, route users, and local ingress names are independent
of the default development lane. Pin all seven ports explicitly; missing,
duplicate, production, and default-development ports fail before installation:

```bash
export AGENT_BROWSER_DEV_NAMESPACE=campaign
export AGENT_BROWSER_DEV_DASHBOARD_PORT=5148
export AGENT_BROWSER_DEV_BACKEND_PORT=5149
export AGENT_BROWSER_DEV_SHADOW_PORT=5150
export AGENT_BROWSER_DEV_LANE_STREAM_PORT=5151
export AGENT_BROWSER_DEV_GUACAMOLE_PORT=8193
export AGENT_BROWSER_DEV_GUACD_PORT=4923
export AGENT_BROWSER_DEV_POSTGRES_PORT=56433
pnpm development-runtime:install
```

Check that those ports are free first. Keep the same namespace and port
environment for status, doctor, provider, and maintenance commands. Review a
separate public HTTPS origin and immutable ingress revision before provider
staging; namespace selection does not publish ingress or authorize reuse of
another environment's routes. Omitting the namespace preserves the default
development installation. Namespaced installation checks production and the
default development identity for non-interference.
...
`stealthcdp_chromium` is worthwhile for the default service posture because it keeps the CDP control plane available while reducing the obvious automation signal exposed by ordinary DevTools-attached Chromium. The built-in Google and Gmail policies therefore use headed stealth Chromium and allow attachable first sign-in. A detached CDP-free first login remains mandatory only when the effective site policy sets `requiresCdpFree: true`. A failed sign-in or challenge can still require operator intervention. An explicit `--browser-build` selection wins over site, profile, preference-binding, and service defaults, including when a service request carries `browserBuild` inside `params`.
...
Service control jobs default to a 900000 ms worker deadline. Set `service.jobTimeoutMs`, pass `--service-job-timeout <ms>`, or set `AGENT_BROWSER_SERVICE_JOB_TIMEOUT_MS` to choose another deadline. A zero or absent configured value selects the built-in default, so every dispatched job has a recovery horizon. Background and explicit service reconciliation mark stranded `running` records `timed_out` after their persisted deadline without retrying the original effect; legacy records without timeout metadata use the same 900000 ms recovery horizon.

If a caller process reaches its own deadline and disconnects first, the daemon promptly releases only that transport connection's profile custody. An already accepted job continues to its recorded terminal state without being replayed. Inspect the retained job before deciding whether any new action is safe.

An authenticated launch-new access plan may reserve a deterministic session route before that session record exists. Execution validates the internal route authorization, current capability, exact profile and owner evidence, and absence of competing live work; the future session does not need to be manually precreated.
...
and the Google and Gmail sign-in policies use headed stealth Chromium for
attachable first sign-in. Remote-headed browser records persist the selected
...
allocations, remote view routes, and route pool entries under
`docs/dev/contracts/`. Browser records and `viewStreams` can carry
`displayAllocationId`, `routeId`, provider concurrency mode, and controller
authority fields so clients can distinguish two private browser routes
...
`GET /api/service/remote-view-routes`, `GET /api/service/route-pool`, and the matching MCP resources
`agent-browser://display-allocations`, `agent-browser://remote-view-routes`,
and `agent-browser://route-pool`. Until a
...
with an exact target ID, and closes the browser through the
...
`view_focus` with an exact target ID before opening the embedded
...
two iframe URLs loaded. It saves service state, route and browser evidence,
...
route-pool entries, runtime convergence, stream URLs, and
...
`service leases` exposes first-class profile lease authority without launching a browser. HTTP `GET /api/service/profile-leases`, MCP `agent-browser://profile-leases`, and generated helpers `getServiceProfileLeases()` and `findServiceProfileLease()` return the same collection. Use `service leases watch` or `watchServiceProfileLeases()` for bounded or abortable polling. Each record names the authenticated principal, exact owner generation, subordinate session and tab work, blocking identity axes, authorized actions, typed continuity recourse, and doctor findings. A current canonical profile claim is the sole operational projection for that profile: retained sessions and owner bindings remain visible context but cannot add doctor blockers. It reports `state=active` with `recourse=continue_with_active_claim`; the claim does not by itself prove that a browser process exists. Use `service leases doctor`, `service leases <id> inspect`, or `service leases <id> explain` for focused diagnosis. Register a service principal with `service leases register --principal-id <id> --profile-id <id> --capability-out <absolute-private-path>`. Registration can happen before or after first launch. Rejoin, renew, and release require the exact current `--revision` and read the capability from `--capability-file`; the raw capability is never accepted on the command line or retained in Service State. When a pre-registered capability later has one exact uncontested ready owner session, or registration finds one live session blocked by `unproven_session_authority`, the lease authorizes `rejoin`. Run `service leases <id> rejoin --revision <revision> --capability-file <path> --expires-at <rfc3339>`. Rejoin may establish the missing binding or compare-and-swap the same principal, profile, and capability binding to a newer current ready owner generation, then binds only that unique exact session and its same-browser active tabs. After an unexpected browser exit, a non-expired registered session work lease preserves this exact profile selection while replay creates the next owner generation. Labels, expired work, and legacy observations cannot cross that gap. Ambiguous sessions, mismatched browsers, and foreign principals fail closed. HTTP mutations require an `Authorization: Bearer` header, while MCP mutation tools accept the ephemeral `profileCapability` field. `service leases <id> reconcile plan` and `planServiceProfileLeaseReconciliation()` return a sealed, expiring descriptor bound to the current boot epoch. When only the same registered capability's owner generation is stale and the exact ready profile, browser, process, and session route still agree, the plan contains one `refresh_principal_owner_binding` transition even when no subordinate session work remains. Apply rechecks every bound identity before advancing that binding and fails closed if the evidence or boot epoch changed. Legacy retained evidence remains observation-only and caller labels cannot grant profile authority. Rows backed only by released or expired legacy sessions report `state=historical`, expose read actions only, have no blocking identity axes, and do not make lease doctor unhealthy. Current nonterminal legacy session or owner evidence still requires identity reconciliation only when no canonical claim exists. Observation-only current lease rows advertise `profile_acquire` when the capability-bound acquisition coordinator is the executable recourse. The dashboard maps `reconcile_plan` to its no-effect Reconcile plan action and offers Acquire profile for `profile_acquire`; it does not reinterpret observation-only evidence as lease mutation authority.
...
If a registered private capability file is lost, inspect only its public authority with `service leases capability-status --principal-id <id> --profile-id <id>`. Rotation is available only when exactly one active grant exists and the profile has no canonical claim or subordinate work. Copy the returned `capabilityId` and `registryRevision` into `service leases rotate-capability --principal-id <id> --profile-id <id> --expected-capability-id <id> --expected-registry-revision <n> --capability-out <absolute-private-path>`. The compare-and-swap mutation revokes only that exact grant, writes one new user-private `0600` file, and rebinds only the same current owner identity when present. It never recovers or prints the old raw capability, accepts a secret on argv, or rotates through HTTP or MCP.
...
Profile access uses a filesystem-like policy. New and ambiguous legacy profiles default to `shared-local`, where a stable self-declared `clientSubjectId` is sufficient and legacy ambiguity is a nonblocking access advisory. When no durable profile is selected and site policy does not require one, the broker assigns that client a deterministic disposable `managed-ephemeral-*` profile and returns an executable launch request. Later labeled requests recover the same self-declared subject and may continue a consistent shared-local session without constructing strict runtime-owner proof. `restricted` requires authenticated ingress and `exclusive` requires a registered profile capability. Use `service_profile_policy_mutate` with `profileId`, `expectedRevision`, and either `mode` plus the `administrator`, `participant`, or `observer` preset, or an exact `targetPolicy`. Narrowing an occupied profile first fences new admission. The caller choosing narrower access must clear incompatible users before the policy commits. Successfully closed tabs remain as history and no longer block the drain; disconnected or otherwise nonterminal tabs still count. Forced cleanup is separate: `service_profile_tab_evict` accepts only an exact `authorizationId` and `tabId` from a persisted eviction plan. Software clients can use `createServiceProfilePolicyMutationRequest()`, `requestServiceProfilePolicyMutation()`, `createServiceProfileTabEvictionRequest()`, and `requestServiceProfileTabEviction()`.
...
A cold shared-local plan supplies a stable `shared-profile-*` logical session derived from the admitted subject and profile. Copy that planned request so execution never inherits unrelated identity from the runtime host's ambient lane.

A registered profile that needs interactive sign-in can use queued service request action `service_profile_manual_seeding_acquire` with `profileId` and `targetServiceId`. It reserves the exact registered profile in SQLite, opens one keeper-bound headed browser without CDP, and returns only an opaque `/remote-view/<handoff-id>` operator handoff after presentation proof. Replays reuse the same live process and handoff. Resolving that URL checks current process and route visibility. For handoffs with a recorded route user, keeper adoption on the same slot, display, and user refreshes the route fence after fresh presentation proof and keeps the same opaque URL. A changed display or route user requires recovery; provider URLs remain internal. If launch identity was uncertain, the profile stays reserved while the recorded PID exists; once that PID is observed absent, acquire terminalizes the old operation and returns `retryRequiresNewOperation: true` without launching again. If a previously ready browser has exited, acquire or handoff resolution closes its exact handoff and returns the same retry marker. Submit a new request ID to retry. After the operator finishes, call `service_profile_manual_seeding_close` with the exact returned `profileId`, `targetServiceId`, `handoffId`, and `pid`. Close verifies and stops only that process, closes its handoff, retains the provider-owned keeper route, and returns separate attachable relaunch and authentication-probe next actions. Visibility is never authentication evidence.
...
Use `service reconcile` to run the persisted browser health and target probes intentionally without requesting a control-plane status snapshot. This command updates the same `reconciliation` snapshot, expires due or orphaned session leases without launching a browser, returns `expiredSessionLeases`, `expiredSessionLeaseCount`, `remoteViewRepair`, and `routePoolRefresh`, refreshes live tab records for reachable browser CDP endpoints, and appends service events. It also completes an owned closing runtime lifecycle only when the exact owner generation still matches, the recorded process group is absent, the registered profile path matches the lifecycle identity, and the profile lock is absent. Live, locked, unmatched, and ambiguous lanes remain unchanged. A later managed launch may replace that exact `terminal/satisfied` owner under a new logical browser ID only by advancing one generation with no pending transfer or lifecycle-key collision; the lifecycle record moves to the new ID and retains one cleanup obligation. The local runtime interlock passes the readiness-verified pool through `--authoritative-route-pool-json <json-array>` so stable route entries track current Guacamole connection and display assignments after a reboot. A conflicting active allocation is retained and reported in `skippedActiveConflictEntryIds` instead of being redirected.
...
`bootEpoch` value identifies the current host boot, and
`bootEpochFindings` reports prior, missing, or unavailable epochs with
`rediscover_current_evidence` recourse. Findings never authorize effects or
cleanup. Browser, session-lease, display, viewer, runtime-lifecycle, PID, and
selected-socket observations capture the epoch that authenticated them; stable
profile, logical browser, route, connection, route-user, and durable handoff
IDs survive reboot. A package-launched or managed-attached Chrome lane is registered with canonical
...
Use `service access-plan` before browser work when an operator, agent, or software client needs the same no-launch broker recommendation as HTTP `GET /api/service/access-plan` and MCP `service_access_plan`. Pass `--session-name <lane>` to select an existing service session or the exact `replacementSessionName` advertised for a cleanup-satisfied terminal owner. The plan expands a unique live-session mapping into both `browserId` and `sessionName`; a missing, ambiguous, incompatible, or non-authorized replacement mapping makes `decision.serviceRequest.available` false instead of falling back to `session:default` or launching an unrelated replacement. Text output includes the selected profile, manual-seeding posture, monitor freshness recommendation, service request availability, and compact `browser_build_summary` explaining which browser build won. Access-plan JSON also includes `decision.profileReuse`, which reports whether the selected profile should reuse an existing compatible live browser, wait for the profile lease with `profileLeasePolicy: "wait"`, or launch a new browser because no compatible acquisition lane exists. A healthy retained browser linked only to inactive sessions reports `rejoin_profile_lease` with blocker `expired_session_recovery_required`; inspect and rejoin the exact profile lease, then request a fresh access plan. `compatibleLiveBrowserCount` counts browsers that can accept the planned operation, while `sameProfileLiveBrowserCount` reports the broader healthy same-profile observation. When the recommendation is `reuse_existing_browser`, the copied `decision.serviceRequest.request` includes top-level `browserId` and `sessionName` route hints so HTTP `POST /api/service/request`, MCP `service_request`, and generated client helpers route ordinary tab and browser commands to the existing daemon lane instead of the service daemon. For `supersede_terminal_owner`, the copied request includes the exact terminal-owner replacement session route. Nonterminal lifecycle and owner rows remain diagnostic evidence. They do not block a cold launch or supply a session identity without a canonical current claim or fresh exact physical collision. Duplicate live browsers or duplicate active leases for the selected profile are surfaced as `duplicatePressure` so clients can queue instead of cloning equivalent runtime profiles. Dashboard launcher eligibility rows use the same `decision.profileReuse` advisory as the row reason when access-plan data has been fetched.

Principal-bound profile reuse requires the profile capability. HTTP callers pass it only as `Authorization: Bearer <profile-capability>` to both `GET /api/service/access-plan` and `POST /api/service/request`. MCP callers pass the ephemeral `profileCapability` field to `service_access_plan` and `service_request`. Generated clients pass `profileCapability` as an option and convert it to the bearer header. Caller labels such as `serviceName` and `agentName` remain attribution only and never grant authority. An authenticated matching principal can reuse its exact retained browser and session. On its first tab request, the broker supplies a stable principal-owned daemon session derived from the public principal and profile identities, prepares that route before relay, and avoids inheriting an unrelated transport default session or its profile lease. Before relay, the broker also attaches an internal route authorization receipt to authenticated cold and terminal-replacement launches. The daemon revalidates the exact capability identity, profile, route, owner generation when present, and absence of competing live authority, so a request reported as available cannot be rejected merely because its prelaunch session record exists before the runtime owner is registered. Capability rotation does not change that route, and raw capability material is never used in its name. A capability bound to one profile cannot cold-launch or reuse a different profile, even when the requested profile has no retained session or lease. When a canonical active claim exists, `decision.profileReuse` reports its claim id, revision, fencing token, and principal id. The matching principal may continue, an unauthenticated caller gets `authenticate_for_profile_reuse`, and a foreign principal gets `wait_for_foreign_principal` even when no session projection exists. Contradictory identity evidence gets `lifecycle_profile_identity_inconsistent`. Capabilities must never appear in URLs, request bodies, command-line arguments, logs, or persisted Service State.
...
Use `service events --limit <n>` to inspect recent reconciliation summaries, browser launch metadata, browser health transitions, browser recovery starts, browser recovery overrides, profile lease wait and lifecycle transitions, tab lifecycle changes, and incident handling events without parsing the full service state. Launch, health, recovery, and profile lease events include `profileId`, `sessionId`, `serviceName`, `agentName`, and `taskName` when that context is known. Add `--kind <kind>`, `--browser-id <id>`, `--profile-id <id>`, `--session-id <id>`, `--service-name <name>`, `--agent-name <name>`, `--task-name <name>`, or `--since <timestamp>` to filter events before the limit is applied. Valid kinds are `reconciliation`, `browser_launch_recorded`, `browser_health_changed`, `browser_recovery_started`, `browser_recovery_override`, `tab_lifecycle_changed`, `profile_lease_wait_started`, `profile_lease_wait_ended`, `profile_lease_lifecycle_changed`, `reconciliation_error`, `incident_acknowledged`, and `incident_resolved`. Profile lease wait events include `details.jobId`, `details.outcome`, `details.conflictSessionIds`, retry timing, and waited timing when known. Profile lease lifecycle events identify registration, rejoin, renewal, or release without retaining the raw capability. `--since` accepts RFC 3339 timestamps.
...
readiness, display access, and route desktop state. Remote-view route checkout
and release actions return typed route metadata. Use
...
`requestServiceRemoteViewRouteRelease()` when a software client needs to
allocate or release a retained Guacamole or RDP route without hand-building the
