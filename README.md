...
the same profile. Service-scoped launches fail before browser start when the
selected profile is already held by another exclusive session; same-session
retained browser reuse remains allowed. MCP typed browser tools accept the same target profile hints,
...
rejected before a service-scoped launch starts another browser. Pass `profile` or `runtimeProfile` only for override
