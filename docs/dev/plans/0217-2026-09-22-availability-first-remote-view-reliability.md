...
2. Stale or ambiguous historical ownership may prevent destructive cleanup of the exact uncertain resource. It cannot prevent selecting, creating, or restarting a separate working route.
...
4. Repeated external desktop and mobile checks cover every configured warm route and show current browser pixels on a clean desktop with correct visibility and z-order.
5. Focus, pointer, keyboard, scrolling, viewport resize, reconnect, and browser, runtime, and route restart recovery remain responsive through the same durable handoff where the contract requires continuity.
...
- Delete or bypass P211 lease-era machinery that can deny ordinary service. Retain only exact non-destructive protection at cleanup boundaries.
...
Install the frozen candidate once in the isolated development runtime under fresh shared-runtime effect custody. Test every warm route from authenticated external desktop and mobile viewports. Exercise focus, pointer, keyboard, scrolling, resize, reconnect, and the required restart cases. Preserve timestamped route and candidate identities plus synthetic, non-private visual evidence. One repair and one repeat of a failed case are allowed; batch related source defects before rebuilding.
...
- Do not revise this plan to explain failed tactics. Update the current-state checkpoint and keep the objective and acceptance contract fixed.
...
| Availability-first open | explicit-close and route residue evidence | stale ownership cannot deny a fresh working route through ordinary open | incomplete |
...
| Desktop rendering | protocol-ready warm routes | repeated current, complete, correctly layered pixels on every warm route | incomplete |
| Desktop and mobile input | none accepted | focus, pointer, keyboard, scroll, resize, and reconnect receipts | incomplete |
| Recovery | component and doctor checks | browser, runtime, and route restart recovery with no stale or blank presentation | incomplete |
| Residue and safety | P211 exact close and process residue checks | fresh process census plus exact non-destructive handling of uncertain resources | incomplete |
