...
After the MCP resource assertion was repaired, CI exposed the same stale assumption in a second Rust unit test:

```text
native::actions::tests::test_service_config_actions_mutate_persisted_state
assertion failed: !store.load().unwrap().site_policies.contains_key("google")
```
...
The action test had the same stale persisted-only assumption. Deleting a configured `google` policy removes the persisted override, but loading service state reapplies the built-in `google` policy. The correct assertion is that the source changes back to `builtin`, not that the effective service state no longer contains `google`.

## Prevention

When changing built-in service policy behavior, source metadata, or MCP/API resource refresh paths, include the MCP site-policy resource test and action mutation test in local validation. Focused smokes that only check generated metadata can miss stale unit expectations in the Rust job.
