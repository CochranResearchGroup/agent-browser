...
Original status: CANDIDATE QUALIFIED | PRODUCTION TRANSACTION NOT APPLIED

Reconciliation status on 2026-08-26: SUPERSEDED BY CURRENT-MAIN
REQUALIFICATION
...
This section records the original preflight against main commit `d07b4811`.
After Plan 0133 advanced main to `f46b9e82`, the source identity and candidate
hash below ceased to be eligible for installation. They remain historical
evidence and must not be used as current transaction inputs.
...
1. `docs/dev/notes/0133-2026-08-25-operator-visible-window-focus-gap-handoff.md`
   prohibited another workstation upgrade while the operator-visible focus-gap
   repair was active. Plan 0133 later reached development acceptance while
   retaining a production-read-only boundary.
