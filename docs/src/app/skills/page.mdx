...
- **agent-browser-service** — Shared and authenticated browser operations: managed profiles, retained-browser reuse, Guacamole/RDP presentation, route capacity, and scoped runtime troubleshooting.
...
npx skills add CochranResearchGroup/agent-browser --skill agent-browser-service
...
Use this focused skill when a browser must retain authentication, share a
managed profile, reuse an existing service-owned browser, or become visible to
an operator through Guacamole or RDP. It teaches agents to begin with
`service_access_plan`, keep browser acquisition separate from operator
presentation and runtime maintenance, and let Agent Browser own leases, route
selection, handoffs, reconciliation, and garbage collection.

For example, a checked-out route pool can block a requested operator view
without blocking ordinary browser automation. Likewise, a global doctor
advisory is maintenance evidence until the requested action or scoped
readiness result classifies it as blocking.
