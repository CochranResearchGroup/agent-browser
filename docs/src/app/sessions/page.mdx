...
The simplest way to reuse your existing Chrome profile naming is to pass a profile name to `--profile`. agent-browser stores that profile in the managed root at `~/.agent-browser/profile` using `--profile-directory`, so cookies and sessions persist across launches.
...
> <strong>Important:</strong> For Chrome password-manager features and some SSO flows, prefer <code>--auto-connect</code> to a real running browser. For regular session persistence, named profiles now persist in <code>~/.agent-browser/profile</code>.
...
<table>
  ...
  <tbody>
    ...
    <tr><td>What is used</td><td>Persistent managed profile under <code>~/.agent-browser/profile</code> with <code>--profile-directory</code></td></tr>
    <tr><td>Persistence</td><td>State is saved across restarts in the managed profile namespace</td></tr>
    <tr><td>Chrome password-store</td><td>Not required for cookie/session persistence; use <code>--auto-connect</code> for real-browser keychain flows</td></tr>
    <tr><td>Windows note</td><td>Using <code>--profile</code> still requires Chrome not holding conflicting named profile locks</td></tr>
  </tbody>
</table>
