...
the patched Chromium posture. The build label does not locate the browser
binary by itself. Provide the current binary location with
<code>executablePath</code> or <code>AGENT_BROWSER_EXECUTABLE_PATH</code>.
<code>agent-browser service status</code> and
<code>GET /api/service/status</code> include a no-launch
<code>launchConfig</code> diagnostic with the resolved executable path, file
existence, and warnings when the selected custom build is not ready.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>service.defaultBrowserBuild</code></td><td>Service launch posture label. Use <code>stealthcdp_chromium</code> with <code>executablePath</code> or <code>AGENT_BROWSER_EXECUTABLE_PATH</code> for the patched Chromium build.</td><td>string</td></tr>
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>AGENT_BROWSER_EXECUTABLE_PATH</code></td><td>Custom browser executable path. Use this or <code>executablePath</code> when <code>service.defaultBrowserBuild</code> is <code>stealthcdp_chromium</code>.</td><td>(auto-discovered browser)</td></tr>
