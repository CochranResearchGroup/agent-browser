...
If you do not set <code>profile</code>, agent-browser uses the managed default profile at <code>~/.agent-browser/profile</code>. Set <code>profile</code> to another persistent directory for dedicated projects, or a Chrome profile name to reuse that name inside the managed <code>~/.agent-browser/profile</code> namespace.
