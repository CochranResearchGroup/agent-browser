...
`GET /api/service/status` returns the same service snapshot as `agent-browser service status`. `GET /api/service/events` accepts `limit`, `kind`, `browser-id`, `browser_id`, `browserId`, and `since` query parameters. Valid event kinds are `reconciliation`, `browser_health_changed`, `tab_lifecycle_changed`, and `reconciliation_error`. `POST /api/service/reconcile` runs the same persisted browser health probes as `agent-browser service reconcile`.
