...
For MCP adapter scaffolding, `agent-browser mcp resources` lists the read-only service resource contracts and `agent-browser mcp read <uri>` reads one resource from persisted service state without launching a browser. Implemented resources are `agent-browser://incidents` and `agent-browser://incidents/{incident_id}/activity`.
