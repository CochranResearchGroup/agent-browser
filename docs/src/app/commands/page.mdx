...
Run <code>pnpm test:service-profile-lookup-no-launch</code> to validate that HTTP <code>/api/service/profiles/lookup</code> selects an authenticated target profile over a target-only profile from seeded temporary service state without launching a browser.
