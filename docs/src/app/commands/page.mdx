...
The same package exposes <code>@agent-browser/client/service-observability</code> for typed service status, collection, config mutation, reconcile, job, event, incident, incident activity, and trace HTTP helpers generated from the matching contract schemas. Collection helpers cover profiles, browsers, sessions, tabs, site policies, providers, and challenges. Config mutation helpers upsert and delete profiles, sessions, site policies, and providers.
