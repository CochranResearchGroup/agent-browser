...
`service cancel <job-id>` marks a queued service job cancelled before it dispatches. This is intentionally conservative: running jobs are not interrupted, and terminal jobs are rejected rather than rewritten. Add `--reason <text>` to record an operator-readable reason.
