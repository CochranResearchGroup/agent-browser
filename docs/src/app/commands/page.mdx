...
For MCP clients, use `mcp serve` to run a read-only stdio server that exposes service resources without launching a browser. The server supports <code>initialize</code>, <code>ping</code>, <code>resources/list</code>, <code>resources/templates/list</code>, and <code>resources/read</code>. For shell inspection, use `mcp resources` to list read-only service resource contracts and `mcp read &lt;uri&gt;` to read one resource from persisted service state. Implemented resources are `agent-browser://incidents`, `agent-browser://profiles`, `agent-browser://sessions`, `agent-browser://browsers`, `agent-browser://tabs`, `agent-browser://jobs`, `agent-browser://events`, and `agent-browser://incidents/{incident_id}/activity`.
