...
Use HTTP <code>GET /api/service/access-plan?service-name=&lt;name&gt;&amp;login-id=&lt;id&gt;</code> or MCP <code>agent-browser://access-plan?serviceName=&lt;name&gt;&amp;loginId=&lt;id&gt;</code> when a caller needs agent-browser to make the no-launch profile, policy, provider, challenge, readiness, and recommended-action decision before requesting a tab or browser action. The readiness decision is scoped to the requested target identities, so an unrelated stale or unseeded login on the same profile does not block the requested site.
