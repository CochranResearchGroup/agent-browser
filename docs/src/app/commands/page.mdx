...
The private workspace package <code>@agent-browser/client</code> generates service request helpers and TypeScript declarations from those schemas. Run <code>pnpm generate:service-client</code> after schema changes, then run <code>pnpm test:service-client-contract</code> and <code>pnpm test:service-client-types</code> to check that generated files and runtime helper types are current.
