...
`service events --limit <n>` prints recent reconciliation summaries and browser health transitions without requiring operators to parse the full service state. Add `--kind <kind>`, `--browser-id <id>`, or `--since <timestamp>` to filter events before the limit is applied. `--since` accepts RFC 3339 timestamps.
