...
Run <code>pnpm test:service-remedies-apply-json-no-launch</code> to validate that <code>service remedies apply --escalation browser_degraded</code> returns the retry response and updates persisted browser state without launching Chrome.
