...
`service reconcile` runs the same persisted browser health probes intentionally without requesting a control-plane status snapshot. It updates `state.json`, refreshes the `reconciliation` snapshot, and returns the reconciled service state plus counts for total and changed browser records.
