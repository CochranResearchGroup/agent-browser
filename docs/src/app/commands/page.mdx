...
The private workspace package <code>@agent-browser/client</code> generates service request helpers and TypeScript declarations from those schemas. It includes <code>createServiceRequest</code>, <code>postServiceRequest</code>, <code>createServiceTabRequest</code>, and <code>requestServiceTab</code> for software clients that want a queued tab by site or login identity. <code>postServiceRequest</code> and <code>requestServiceTab</code> return <code>ServiceRequestResponse</code>, the standard service command envelope with <code>success</code>, optional <code>data</code>, optional <code>error</code>, optional <code>warning</code>, and action-specific fields. Run <code>pnpm generate:service-client</code> after schema changes, then run <code>pnpm test:service-client</code> to check generated files, runtime helper types, package export resolution, typed service command responses, service request helpers, and observability helpers. Use <code>pnpm test:service-client-contract</code>, <code>pnpm test:service-client-types</code>, <code>pnpm test:service-client-exports</code>, <code>pnpm test:service-request-client</code>, or <code>pnpm test:service-observability-client</code> when only one client contract needs validation.
