...
`service status` is read-only. It reports the service control-plane state and configured service-mode entities from `agent-browser.json` and `~/.agent-browser/config.json` without launching a browser.
