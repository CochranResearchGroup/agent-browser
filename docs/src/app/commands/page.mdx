...
Run <code>pnpm test:service-remedies-apply-json-no-launch</code> to validate that <code>service remedies apply --escalation browser_degraded</code> and <code>--escalation os_degraded_possible</code> return retry responses and update persisted browser state without launching Chrome.
