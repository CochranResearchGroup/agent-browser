...
Run <code>pnpm test:service-access-plan-no-launch</code> to validate that HTTP <code>/api/service/access-plan</code>, MCP <code>agent-browser://access-plan</code>, and <code>getServiceAccessPlan()</code> return the same recommendation from seeded temporary service state without creating browsers or browser-launching jobs.
